...
/*------------------------------------------------------------------------------
	...
	"CMOL.c" : programa de test de usuario progG;
				(versión 1.0)
				
	Este programa calcula el peso molecular de un compuesto químico
...
typedef struct {
    ...
    int atomic_weight;          // Peso atómico del elemento (en unidades de 0.0001 g/mol)
} Element;

// Definimos una tabla periódica con algunos elementos y sus pesos atómicos (multiplicados por 10,000)
...
Element periodic_table[] = {
    ...
    {"Db", 2620000}, {"Bh", 2640000}, {"Sg", 2660000}, {"Mt", 2680000},
    {"Hs", 2770000}
>>>>>>> ceb5f2d16c54e4322c0eec3c7b89c7ff64420c4b
};

// Función para imprimir un número en formato de coma fija con 4 decimales
void printComaFija(int num) {
<<<<<<< HEAD
    unsigned int acc = 10000;					// Factor de precisión para coma fija
	unsigned int quo, mod;						// Variables para cociente y residuo
	//unsigned int integer = num * 10000;			// Conversión a entero de precisión
	
	int integer_part = num / 10000;			// Parte entera del número

	// Obtener la parte decimal con GARLIC_divmod
	GARLIC_divmod(num, acc, &quo, &mod);

    // Imprimir la parte entera y decimal, ajustando para cuatro dígitos de precisión
    GARLIC_printf("El peso molecular es: %d.%d g/mol\n", integer_part, mod);
=======
    int integer_part = num / 10000;   // Parte entera del número
...
void _start(int arg) {
<<<<<<< HEAD
    int total_weight = 0;               // Peso molecular total acumulado (en unidades de 0.0001 g/mol)
    ...
    const char *formula = "H2O";       // Fórmula química a evaluar (ejemplo: Cloruro de Sodio)
