...
_ga_fopen:
   push {r4, lr}            @; Guardar r4 y lr en la pila
    ...
    @; Pasar parámetros a fopen según convención de llamada ARM
    mov r0, r0                @; filename ya está en r0
...
_ga_fclose:
    ...
    @; Pasar el puntero del archivo a fclose
...
_ga_fread:
    ...
    @; Pasar parámetros a fread según la convención de llamada ARM
    mov r0, r0               @; buffer (destino) en r0
    mov r1, r1               @; tamaño de cada elemento en r1
    mov r2, r2               @; número de elementos a leer en r2
